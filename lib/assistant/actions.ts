import type { AssistantBookingDraft, AssistantDecision, AssistantOrderDraft } from "../ai/openai";
...
async function createOrder(context: AgentContext, order: AssistantOrderDraft): Promise<AssistantActionResult> {
  ...
  const rows = await supabaseDataRequest<UnknownRow[]>("orders", {
    method: "POST",
    body: JSON.stringify({
...
export async function executeAssistantDecision(context: AgentContext, decision: AssistantDecision): Promise<AssistantActionResult> {
  if (decision.intent === "order" && decision.order) return createOrder(context, decision.order);
  if (decision.intent === "booking" && decision.booking) return createBooking(context, decision.booking);
