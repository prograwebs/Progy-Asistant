...
import { executeTool, getEnabledToolsForBusiness } from "@/lib/server/agent/tools/registry";
import { MAX_DEMO_QUESTIONS, normalizeDemoQuestion } from "@/lib/shared/assistant/demo-limits";
import { developmentTestingMode, entitlementsFor, normalizePlanCode, voiceTrialAllowance } from "@/lib/server/billing/entitlements";
import { checkQuota } from "@/lib/server/billing/quota";
import { exceedsBase64SourceLimit, exceedsPayloadLimit, MAX_PAYLOAD_MB } from "@/lib/shared/config/limits";
import { loadAgentContext, SupabaseDataError, supabaseDataRequest } from "@/lib/server/data/supabase";
import { recordElevenLabsUsage, recordOpenAIUsage } from "@/lib/server/usage/ledger";
import { resolveOnboardingVoiceId, synthesizeSpeech, VoiceServiceError } from "@/lib/server/voice/elevenlabs";
import { finishTurnTrace, startTurnTrace, type TurnTrace } from "@/lib/server/observability/langfuse";
