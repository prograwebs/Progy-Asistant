    ...
    <div className={styles.grid}>
      <Card className={styles.cardHalf} title="Preparación para publicar" description="El mismo checklist puede repetirse después de cada cambio importante del asistente." tag={launchReady ? "Listo" : `${readiness.filter(([, done]) => done).length}/${readiness.length}`}>
        <div className={styles.list}>
          {readiness.map(([label, done, section]) => <div className={styles.listRow} key={label}><div><b>{done ? "✓" : "○"} {label}</b><small>{done ? "Validado" : "Completa este paso antes de publicar."}</small></div>{!done && <button className={styles.textButton} onClick={() => onGo(section)}>Configurar →</button>}</div>)}
        </div>
      </Card>
